...
public class Calculations
{
	...
	public double t1(double x) throws IllegalArgumentException
	{
		double a=0.1, b=0.2, t1=0;
		if(x<=0)
			throw new IllegalArgumentException("Аргумент под знаком логрифма должен быть больше нуля.");
		t1=(1/Math.pow(a,3))*(Math.log(x)+(2*b/x)-Math.pow(b,2)/(2*Math.pow(x,2)));
		return t1;
	}
	public double t2(double x) throws IllegalArgumentException 
	{
		double a=0.1, t2=0;
